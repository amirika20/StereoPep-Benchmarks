"""
GIN trained from scratch benchmark for the DIA dataset.

Same architecture as benchmarks/gin_scratch.py (Hu et al., ICLR 2020),
adapted for local DIA retention-time data.  Identical to pretrained_gin.py
but all weights are randomly initialised — no external checkpoint.
SMILES are generated from sequences using RDKit's MolFromFASTA.
No diastereomer/tag/substitution pair metrics.

Results written to benchmarks_dia/output/results_gin_scratch_dia_seed{N}.json.
"""
...
def smiles_to_graph(smiles: str) -> Data | None:
    ...
    if mol is None or mol.GetNumAtoms() == 0:
        return None
    atom_feats = [[min(a.GetAtomicNum() - 1, 117), _CHIRALITY_MAP.get(a.GetChiralTag(), 0)]
...
def main() -> None:
    ...
    weights_path = WEIGHTS_DIR / f"results_gin_scratch_dia_seed{seed}.pt"
    print(f"\n── Seed {seed} ──")
    ...
    save_results(seed, test_metrics, train_metrics, training, config,
                 RESULTS_DIR, "results_gin_scratch_dia")

...
Results are written to benchmarks/results_deeprt_capsnet.txt.
"""
...
RESULTS_FILE  = Path(__file__).parent / "results_deeprt_capsnet.txt"
...
def write_results(
    ...
    lines = [
        "=" * 70,
        "DeepRT CapsNet (Tang et al., 2020) — Retention Time Prediction",
        f"Dataset : {HF_REPO}",
        ...
        "Model configuration",
        "-" * 40,
        f"  Architecture        : Embedding + 2×Conv2d + PrimaryCaps + DigitCaps (ensemble)",
        f"  Kernel sizes        : {KERNEL_SIZES}  (one model each, averaged)",
        f"  Embedding dim       : {EMB_SIZE}",
        f"  Conv filters        : {CONV_FILTERS}",
        f"  Primary capsules    : 8 dims × 32 capsules",
        f"  Digit capsules      : 1 capsule × 16 dims",
        f"  Routing iterations  : {NUM_ROUTING}",
        ...
        f"  Device              : {DEVICE}",
        f"  Target normalisation: min={y_min:.4f}  max={y_max:.4f}  (train split)",
        "",
...
def main() -> None:
    ...
    # Normalise to [0, 1] — the CapsNet RSS output is bounded in [0, 1)
    ...
    for K in KERNEL_SIZES:
        print(f"\n[train] kernel_size={K}  "
              f"digit_nodes={_digit_nodes(MAX_LEN, K)}")
    ...
    write_results(test_metrics, stereo_metrics, histories, n_params_each, y_min, y_max)

...
def get_pharm_label(frag: str):
    # NOTE: this closed 258-fragment vocab is only meaningful for the masked
    # fragment-identity pretraining objective. The value returned here is
    # never attached to the graph (see the commented-out
    # `g.nodes['p'].data['label'] = ...` below in Mol2HeteroGraph), so it has
    # no effect on the embeddings extracted via PepLandFeatureExtractor.
    # Silenced to avoid log spam — common residues (e.g. protonated Lys/His
    # side chains) routinely miss this vocab since it was likely built from
    # neutral-form fragment SMILES.
